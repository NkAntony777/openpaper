...
def _core_word_count(
    ...
    if intro_words >= targets['intro']:
        ...
        score += 4
        issues.append(QualityIssue('introduction', 'word_count', str(intro_words),
                                   str(targets['intro']), 'warning',
                                   f"Introduction short: {intro_words} words (target: {targets['intro']})"))
    else:
        issues.append(QualityIssue('introduction', 'word_count', str(intro_words),
                                   str(targets['intro']), 'error',
    ...
    if body_words >= targets['body']:
        ...
        score += 6
        issues.append(QualityIssue('body', 'word_count', str(body_words),
                                   str(targets['body']), 'warning',
                                   f"Body short: {body_words} words (target: {targets['body']})"))
    else:
        issues.append(QualityIssue('body', 'word_count', str(body_words),
                                   str(targets['body']), 'error',
    ...
    if conclusion_words >= targets['conclusion']:
        ...
        score += 2
        issues.append(QualityIssue('conclusion', 'word_count', str(conclusion_words),
                                   str(targets['conclusion']), 'warning',
                                   f"Conclusion short: {conclusion_words} words (target: {targets['conclusion']})"))
    else:
        issues.append(QualityIssue('conclusion', 'word_count', str(conclusion_words),
                                   str(targets['conclusion']), 'error',
...
def _core_citations(
    all_text: str, min_citations: int
) -> Tuple[int, List[QualityIssue]]:
    ...
    if unique_citations >= min_citations:
        ...
        score += 8
        issues.append(QualityIssue('citations', 'unique_citations', str(unique_citations),
                                   str(min_citations), 'warning',
                                   f"Few unique citations: {unique_citations} (target: {min_citations})"))
    else:
        issues.append(QualityIssue('citations', 'unique_citations', str(unique_citations),
                                   str(min_citations), 'error',
    ...
    if total_citations >= expected_density:
        ...
        score += 5
        issues.append(QualityIssue('citations', 'citation_density', f"{total_citations} refs / {word_count} words",
                                   f">= {expected_density} refs", 'warning',
                                   f"Low citation density: {total_citations} refs in {word_count} words"))
    else:
        issues.append(QualityIssue('citations', 'citation_density', f"{total_citations} refs / {word_count} words",
                                   f">= {expected_density} refs", 'error',
                                   f"Very low citation density: {total_citations} refs in {word_count} words"))

    return score, issues


def _core_completeness(
    ...
    for name, content in sections:
        ...
        if stripped and len(stripped) > 100:
            ...
            score += 2
            issues.append(QualityIssue(key, 'completeness', f"{len(stripped)} chars",
                                       '> 100 chars', 'warning', f"{name} section too brief"))
        else:
            issues.append(QualityIssue(key, 'completeness', 'missing',
                                       'present', 'error', f"Missing {name} section"))
...
def _core_structure(all_text: str) -> Tuple[int, List[QualityIssue]]:
    ...
    if len(headers) >= 5:
        ...
        score += 5
        issues.append(QualityIssue('structure', 'headers', str(len(headers)),
                                   '>= 5', 'warning', f"Few section headers: {len(headers)}"))
    else:
        issues.append(QualityIssue('structure', 'headers', str(len(headers)),
                                   '>= 5', 'error', "Missing section headers"))
    ...
    if len(paragraphs) >= 10:
        ...
        score += 2
        issues.append(QualityIssue('structure', 'paragraphs', str(len(paragraphs)),
                                   '>= 10', 'warning', f"Few paragraphs: {len(paragraphs)}"))
    else:
        issues.append(QualityIssue('structure', 'paragraphs', str(len(paragraphs)),
                                   '>= 10', 'error', "Very few paragraphs"))
